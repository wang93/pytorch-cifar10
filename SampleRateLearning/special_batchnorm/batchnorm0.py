...
class _BatchNorm(origin_BN):
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.training:
            ...
            y = (input - di_mean.view(new_size)) \
                / torch.full_like(di_var, self.eps).sqrt().add(di_var.sqrt()).view(new_size)
            ...
            y = (input - self.running_mean.view(new_size)) \
                / torch.full_like(self.running_var, self.eps).sqrt().add(self.running_var.sqrt()).view(new_size)
